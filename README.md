...
본 플랫폼을 고객사 환경에 완벽히 설치하고 실행하기 위한 마스터 설치 시퀀스입니다. 1단계부터 순서대로 막힘없이 따라 하시면 즉시 대시보드가 정상 기동됩니다.

---

### 1️⃣ [Step 1] GCP 클라우드 사전 데이터 파이프라인 구성 (사전 필수)
...
1. **GCP Billing Export 활성화**:
   ...
   - 자동 생성된 `gcp_billing_export_resource_v1_{BILLING_ACCOUNT_ID}` 테이블명을 확인해 둡니다.
2. **Log Router 감사 로그 싱크 구성**:
   - Logging 콘솔 ➡️ 로그 라우터(Log Router)에서 BigQuery dataset 목적지 싱크를 만듭니다.
   - 본 문서 **2.2절에 작성된 로그 인클루전 필터 쿼리**를 주입하여 `ge_analytics` 등의 데이터셋에 로그가 자동 적재되도록 설정합니다.

---

### 2️⃣ [Step 2] 로컬 샌드박스 환경 설정 및 로컬 구동 (Local Verification)
클라우드 배포 전에 로컬 장비에서 정상 쿼리 작동 여부를 즉시 검증합니다.

1. **config.yaml 환경설정 튜닝**:
   - 프로젝트 루트의 `config.yaml` 파일을 열고, [Step 1]에서 생성한 실측 GCP 자원 정보들로 플레이스홀더를 수정합니다:
     - `gcp.project_id`: "사용자의 실측 GCP 프로젝트 ID"
     - `gcp.audit_dataset_id`: "감사 로그가 쌓이는 빅쿼리 데이터셋 ID (E.g. ge_analytics)"
     - `gcp.billing.dataset_id`: "빌링 데이터셋 ID (E.g. billing_detailed_usage)"
     - `gcp.billing.table_id`: "gcp_billing_export_resource_v1_{BILLING_ACCOUNT_ID}"
     - `gcp.billing.account_id`: "GCP 결제 계정 ID (E.g. 01E9C5-XXXXXX)"
2. **GCP 로컬 인증 토큰 바인딩**:
   - 로컬 터미널에서 아래 명령을 실행하여 브라우저 로그인을 마치고 Application Default Credentials(ADC)를 활성화합니다:
     ```bash
     gcloud auth application-default login
     ```
3. **가상환경 구성 및 로컬 백엔드 서버 구동**:
   ```bash
   python3 -m venv venv
   ...
   ```
4. 웹 브라우저를 열고 `http://localhost:8088/` 에 접속하여 정상 기동을 눈으로 검증합니다.

---

### 3️⃣ [Step 3] 프로덕션 클라우드 배포 (선택형 배포 실행)
로컬 검증을 완료한 후, 아래의 두 가지 방법 중 선호하는 방식을 선택하여 Google Cloud Run으로 무중단 프로덕션 릴리즈를 수행합니다.

#### 🎯 Option A: Terraform IaC 자동 인프라 배포 (적극 권장)
인프라를 코드로 일괄 자동 관리 및 프로비저닝하는 가장 강력하고 심플한 방법입니다.

1. **배포 변수 파일 (`terraform.tfvars`) 작성**:
   - 프로젝트 루트에 `terraform.tfvars` 파일을 생성하고 아래 템플릿의 빈칸을 고객사 정보로 채웁니다:
     ```hcl
     # terraform.tfvars
...
2. **테라폼 명령어 순차 실행**:
   ...
   ```
3. **결과 확인**:
   - 배포 완료 시 화면에 출력되는 `dashboard_url` 주소를 획득하여 즉시 브라우저에서 관제를 시작합니다.

#### 🎯 Option B: gcloud CLI 수동 원스톱 빌드 & 배포 (대안)
테라폼 도구가 설치되어 있지 않은 환경에서 gcloud 명령어로 대화식/원스톱 배포를 수행합니다.

1. 아래 배포 명령을 실행하여 로컬 소스 코드를 GCP Artifact Registry 상에서 클라우드 원격 빌딩하고 배포를 실행합니다:
   ```bash
   ...
   ```
2. 배포 완료 후 제공되는 **Service URL**을 통해 대시보드 및 AI 분석 챗봇을 즉각 활성화합니다.
