...
## 🏗️ 6. Terraform IaC 자동 인프라 배포 가이드 (Infrastructure as Code)

본 프로젝트는 고객사 인프라 관리자가 GCP 환경에 원클릭으로 대시보드 구동용 보안 계정(SA), IAM 최소 권한, 그리고 Google Cloud Run 서비스를 배포할 수 있도록 Terraform 구성 파일(`main.tf`, `variables.tf`)을 제공합니다.

### 6.1 IaC 배포 변수 설정 (`terraform.tfvars`)
배포를 시작하기 전에, 프로젝트 루트에 `terraform.tfvars` 파일을 생성하고 아래와 같이 대상 GCP 환경의 파라미터 값들을 기입합니다:
...
### 6.2 IaC 배포 명령 실행 순서
로컬 터미널에서 아래 테라폼 표준 명령어를 실행하여 원스톱으로 클라우드 인프라를 프로비저닝합니다:

1. **테라폼 초기화 (Provider 및 모듈 다운로드)**:
   ```bash
   terraform init
   ```

2. **배포 계획 검증 (시뮬레이션 및 권한 체크)**:
   ```bash
   terraform plan
   ```

3. **GCP 실측 인프라 생성 및 배포 적용**:
   ```bash
...
> [!TIP]
> **배포 완료 아웃풋**:
> 배포가 성공적으로 완료되면 터미널 화면에 `dashboard_url = "https://ai-governance-dashboard-xxx.run.app"` 형태로 대시보드 고유 HTTPS 라이브 주소가 자동으로 즉시 출력됩니다.
